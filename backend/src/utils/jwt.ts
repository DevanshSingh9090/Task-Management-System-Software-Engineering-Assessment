...
export const verifyAccessToken = (token: string): AppJwtPayload => {
  return jwt.verify(token, config.JWT_ACCESS_SECRET as string) as AppJwtPayload;
};

export const verifyRefreshToken = (token: string): AppJwtPayload => {
  return jwt.verify(token, config.JWT_REFRESH_SECRET as string) as AppJwtPayload;
};